...
import { OutputType } from "../Boltz.js";
import { Errors } from "../consts/Errors.js";
import { isSwapTree, } from "../consts/Types.js";
import { compareInputs } from "./Bip69.js";
import { createControlBlock, taprootHashTree } from "./TaprootUtils.js";
const LEGACY_SIGHASH = SigHash.ALL;
const DUMMY_TAPROOT_SIGNATURE = new Uint8Array(64);
export const isRelevantTaprootOutput = (utxo) => utxo.type === OutputType.Taproot && utxo.cooperative !== true;
export const validateInputs = (utxos) => {
    if (utxos
...
export const getClaimLeaf = (swapTree) => {
    if (!isSwapTree(swapTree)) {
        throw new Error(Errors.claimRequiresClaimLeaf);
    }
    return swapTree.claimLeaf;
};
export const getTapLeaf = (isRefund, swapTree) => isRefund ? swapTree.refundLeaf : getClaimLeaf(swapTree);
export const constructClaimTransaction = (utxos, destinationScript, fee, isRbf = true, timeoutBlockHeight, isRefund = false) => {
    ...
    const inputSum = sortedUtxos.reduce((acc, utxo) => acc + utxo.amount, BigInt(0));
    tx.addOutput({
        amount: inputSum - fee,
    ...
    for (const [index, utxo] of sortedUtxos.entries()) {
        switch (utxo.type) {
            case OutputType.Legacy: {
                ...
                const hash = tx['preimageLegacy'](index, utxo.redeemScript, LEGACY_SIGHASH);
        ...
        if (utxo.type === OutputType.Taproot) {
            ...
            const sigHash = tx.preimageWitnessV1(index, sortedUtxos.map((out) => out.script), SigHash.DEFAULT, sortedUtxos.map((out) => out.amount), undefined, tapLeaf.output, tapLeaf.version);
            ...
            witness.push(createControlBlock(taprootHashTree(utxo.swapTree.tree), tapLeaf, utxo.internalKey));
            tx.updateInput(index, {
                finalScriptWitness: witness,
            });
        }
        else if (utxo.type === OutputType.Bech32 ||
            utxo.type === OutputType.Compatibility) {
            const sigHash = tx.preimageWitnessV0(index, utxo.redeemScript, LEGACY_SIGHASH, utxo.amount);
            tx.updateInput(index, {
                finalScriptWitness: [
...
export const signLegacy = (hash, privateKey) => {
    return concatBytes(signECDSA(hash, privateKey, true), new Uint8Array([LEGACY_SIGHASH]));
};
