...
import { Script, TaprootControlBlock, } from '@scure/btc-signer';
import { TAP_LEAF_VERSION, tapLeafHash, } from '@scure/btc-signer/payment.js';
import { compareBytes, equalBytes, taprootTweakPubkey, } from '@scure/btc-signer/utils.js';
...
export const swapLeafsToTree = (claimLeaf, refundLeaf) => [claimLeaf, refundLeaf];
export const createControlBlock = (hashTree, leaf, internalKey) => {
    const paths = taprootAddPath(hashTree);
    const path = taprootWalkTree(paths).find((path) => equalBytes(path.script, leaf.output));
    if (path === undefined) {
...
export const createLeaf = (isLiquid, script) => ({
...
export const taprootHashTree = (tree) => {
...
const taprootAddPath = (tree, path = []) => {
    if (tree.type === 'leaf') {
        return { ...tree, path };
    }
    return {
